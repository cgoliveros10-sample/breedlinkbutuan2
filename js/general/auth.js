// auth.js - Complete working version with proper initialization
...
const User = {
    ...
    async _getSupabase() {
        ...
        this._supabase = await waitForSupabase();
        return this._supabase;
    },

    async login(email, password) {
        const supabaseClient = await this._getSupabase();
        if (!supabaseClient) {
...
async function handleSignup(event) {
    ...
    if (typeof Validators !== 'undefined') {
        if (!Validators.name(name)) {
            if (typeof showToast === 'function') showToast('Please enter a valid name (≥2 characters)', 'error');
            return;
        }
    ...
    if (!terms) {
        if (typeof showToast === 'function') showToast('Please accept the Terms of Service', 'error');
        return;
    }
    ...
    try {
        ...
        if (typeof showToast === 'function') showToast(`Welcome to BreedLink, ${user.name}! 🎉`);
        setTimeout(() => window.location.href = 'profile.html', 800);
    } catch (error) {
        if (typeof showToast === 'function') showToast(error.message, 'error');
        if (btn) {
